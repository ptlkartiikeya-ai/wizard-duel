...
class NetworkManager {
  ...
  initElements() {
    ...
    // NOTE: player-name input was removed; name now comes from logged-in profile
  ...
  bindEvents() {
    ...
    this.elementButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // Also update parent class-card highlight
        document.querySelectorAll('.class-card').forEach(c => c.classList.remove('active'));
        const parentCard = e.currentTarget.closest('.class-card');
  ...
  // (duplicate initElements removed — defined once above)

  // --- Player Settings Getter/Setter ---

  getCustomization() {
    const activeColorBtn = document.querySelector('.color-btn.active');
    // Prefer class-card.active > element-btn, then fallback to element-btn.active
    const activeCard = document.querySelector('.class-card.active');
    const activeElementBtn = activeCard
      ? activeCard.querySelector('.element-btn')
      : document.querySelector('.element-btn.active');
    ...
    // Name always comes from the logged-in profile
    const name = (window.dbManager && window.dbManager.currentUser)
      ? window.dbManager.currentUser.username
      : 'Spellcaster';
    
    let wizardLevel = 1;
    if (window.dbManager && window.dbManager.currentUser && window.dbManager.currentUser.wizardLevels) {
